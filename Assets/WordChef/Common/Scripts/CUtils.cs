...
public class CUtils {
    ...
    public static void BackScene(int sceneIndex,MonoBehaviour behaviour=null) {
            ScreenFader.instance.BackScene (sceneIndex);
    }
    public static void LoadScene(int sceneIndex, bool useScreenFader = false, MonoBehaviour behaviour=null) {
         SceneController.Instance.AddScene ();
        if (DialogController.instance.IsDialogShowing()) {
            DialogController.instance.CloseCurrentDialog ();
        }
        if(sceneIndex==1 && behaviour != null) {
            LoadSceneAsy (behaviour,sceneIndex,useScreenFader);
        } else if (useScreenFader) {
    ...
    public static void LoadSceneAsy(MonoBehaviour behaviour, int sceneIndex,bool useScreenFader=false ) {
         behaviour.StartCoroutine (Load (sceneIndex, useScreenFader ));
       
    }
    private static IEnumerator Load(int sceneIndex,bool useScreenFader = false) {
        AsyncOperation op = SceneManager.LoadSceneAsync (sceneIndex);
        op.allowSceneActivation = false;
        Scene scene = SceneManager.GetSceneByBuildIndex (sceneIndex);

        while (op.progress<0.9f) {
            Debug.Log ("Operation Progress : "+op.progress);
            yield return null;
        }
